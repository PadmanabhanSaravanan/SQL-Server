...
7. [**Stored Procedure Output Parameters**](#stored-procedure-output-parameters)
8. [**Stored Procedure Handling Exceptions**](#stored-procedure-handling-exceptions)
...
* Exceptions are created with CREATE EXCEPTION , modified with ALTER EXCEPTION , and dropped with DROP EXCEPTION . 

* A stored procedure raises an exception with EXCEPTION <name>.

Stored procedure handling exception as follows:
...
```markdown
CREATE PROCEDURE employee.insertEmployee(
	@first_name VARCHAR
	)
AS
BEGIN TRY
 INSERT INTO employee.employees(first_name)
 VALUES(@first_name)
END TRY
BEGIN CATCH
...
![image output](image/output148.PNG)

EXEC employee.insertEmployee Aravind;

![image output](image/output149.PNG)
...
```markdown
CREATE PROCEDURE employee.insertEmployee1(
	@last_name VARCHAR
	)
AS
BEGIN TRY
 INSERT INTO employee.employees(last_name)
 VALUES(@last_name)
END TRY
BEGIN CATCH
...
![image output](image/output150.PNG)

```markdown
EXEC employee.insertEmployee1 h;
```

![image output](image/output151.PNG)
...
```markdown
CREATE PROCEDURE employee.insertEmployee2(
	@last_name VARCHAR
	)
AS
BEGIN TRY
...
![image output](image/output152.PNG)

```markdown
EXEC employee.insertEmployee2 '';
```

![image output](image/output153.PNG)
